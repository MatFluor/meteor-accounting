...
class TreeOfTableModel(QAbstractItemModel):
    ...
    def load(self, filename, nesting, separator): 
        ...
        fh = None
        try:
            for line in filename:#codecs.open(unicode(filename), "rU", "utf8"):
                if not line:
                    continue
    ...
    def data(self, index, role):
        if role == Qt.TextAlignmentRole:
            return unicode(int(Qt.AlignTop|Qt.AlignLeft))
        if role != Qt.DisplayRole:
            return #unicode()
        node = self.nodeFromIndex(index)
        assert node is not None
        if isinstance(node, BranchNode):
            return (unicode(node.toString())
                    if index.column() == 0 else unicode(""))
        return unicode(node.field(index.column()))
    ...
    def headerData(self, section, orientation, role):
        if (orientation == Qt.Horizontal and
            ...
            return unicode(self.headers[section])
        return unicode()
